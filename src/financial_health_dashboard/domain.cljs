...
(defn user-accounts
  "Return a user's accounts, filtered if account-type is provided"
  ([{:user/keys [accounts]}]
   accounts)
  ([data account-type]
   (filter #(= (:account/type %) account-type) (user-accounts data))))
...
(defn account-balances
  "Returns all account balances, filtered if year and month are provided"
  ([data]
